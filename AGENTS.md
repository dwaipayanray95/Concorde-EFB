...
- Monochromatic Aviation Amber Redesign: Restyled app-wide palette with Signature Aviation Amber (`#F59E0B` dark / `#D97706` light) and deep obsidian/low-reflection gunmetal surfaces (`#090B10` canvas, `#131722` surface, `#191F2D` inset results). Material 3 squircle amber indicators on `EfbNavRail`, amber indicator pips on `EfbCard` titlebars, 1px precision bezel borders on cards, amber focus rings on form inputs, and high-contrast V-Speed tiles with unit badges.
