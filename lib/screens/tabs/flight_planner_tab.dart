...
class FlightPlannerTab extends ConsumerStatefulWidget {
  const FlightPlannerTab({super.key});

  @override
  ConsumerState<FlightPlannerTab> createState() => _FlightPlannerTabState();
}

class _FlightPlannerTabState extends ConsumerState<FlightPlannerTab> {
              ...
              children: [
                Expanded(
                  child: ref.watch(departureMetarFutureProvider).when(
                    data: (metar) => _buildMetarDisplay(ref, 'DEP METAR', metar, ref.watch(departureRunwayProvider), showDepRaw, () => setState(() => showDepRaw = !showDepRaw)),
                    loading: () => const SizedBox(height: 140, child: Center(child: CircularProgressIndicator())),
                    error: (error, stack) => _buildMetarDisplay(ref, 'DEP METAR', '', ref.watch(departureRunwayProvider), showDepRaw, () => setState(() => showDepRaw = !showDepRaw)),
                  ),
                ),
                const SizedBox(width: 32),
                Expanded(
                  child: ref.watch(arrivalMetarFutureProvider).when(
                    data: (metar) => _buildMetarDisplay(ref, 'ARR METAR', metar, ref.watch(arrivalRunwayProvider), showArrRaw, () => setState(() => showArrRaw = !showArrRaw)),
                    loading: () => const SizedBox(height: 140, child: Center(child: CircularProgressIndicator())),
                    error: (error, stack) => _buildMetarDisplay(ref, 'ARR METAR', '', ref.watch(arrivalRunwayProvider), showArrRaw, () => setState(() => showArrRaw = !showArrRaw)),
                  ),
                ),
              ],
            ),
  ...
  Widget _buildMetarDisplay(WidgetRef ref, String title, String metarStr, Runway? runway, bool showRaw, VoidCallback onToggle) {
