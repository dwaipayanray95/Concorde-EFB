...
const ftToM = (ft) => {
    const value = typeof ft === "number" ? ft : parseFloat((ft ?? "").toString().trim() || "0");
    return value * 0.3048;
};
...
function buildWorldAirportDB(airportsCsvText, runwaysCsvText) {
    ...
    for (const r of runwaysRows) {
        ...
        const parsedLength = Number.isFinite(lengthMValue) && lengthMValue > 0 ? lengthMValue : ftToM(r?.length_ft ?? null);
        ...
        if (leIdent)
            airport.runways.push({ id: leIdent, heading: Math.round(Number.isFinite(leHdg) ? leHdg : 0), length_m: lengthM });
        if (heIdent)
            airport.runways.push({ id: heIdent, heading: Math.round(Number.isFinite(heHdg) ? heHdg : 0), length_m: lengthM });
    }
    return airportsMap;
}
...
function StatPill({ label, value, ok = true }) {
    return (_jsxs("div", { className: `px-2 py-1 rounded-full text-xs font-mono border ${ok ? "border-emerald-500/40 text-emerald-300" : "border-rose-500/40 text-rose-300"}`, children: [label, ": ", _jsx("span", { className: "font-bold", children: value })] }));
}
function HHMM({ hours }) {
...
function runSelfTests() {
    const results = [];
    try {
        results.push({ name: "cruiseTimeHours(1164,1164)=1h", pass: approxEqual(cruiseTimeHours(1164, 1164), 1.0) });
    }
    catch (e) {
    ...
    try {
        ...
        results.push({ name: "altitudeBurnFactor bounds", pass: f1 >= 1.19 && f1 <= 1.21 && f2 >= 0.99 && f2 <= 1.01, values: { f1, f2 } });
    }
    catch (e) {
    ...
    try {
        ...
        results.push({ name: "windComponents 90° crosswind ≈ 20 kt", pass: Math.abs((comps.crosswind_kt ?? 0) - 20) < 0.1, value: comps });
    }
    catch (e) {
...
function ConcordePlannerCanvas() {
    ...
    const [arrRw, setArrRw] = useState("");
        ...
        const [version, setVersion] = useState("v0.83");
...
class ErrorBoundary extends React.Component {
    ...
    componentDidCatch(error, info) { console.error('Concorde EFB crashed:', error, info); }
    render() {
        if (this.state.error) {
